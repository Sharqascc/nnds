...
class LinearNoiseScheduler:
    ...
    def add_noise(self, x, t, noise):
        ...
        sqrt_one_minus_alpha_bar = self.sqrt_one_minus_alpha_cumprod[t].view(-1, 1, 1, 1).to(x.device)
...
class TrajectoryUNet1D(nn.Module):
    ...
    def forward(self, x, t, cond):
        ...
        c_emb = self.cond_embed(cond.view(batch, -1))
...
def build_velocity_tensors(csv_path, Th=16):
    ...
    return vel_tensor_norm, cond_tensor, mu, sigma, gt_trajs, torch.from_numpy(start_pos_arr).float()


# ============ Training ============

def train_model(train_csv, input_dim=32, hidden_dim=128, epochs=100, batch_size=16, lr=1e-4, save_path="checkpoints/best.pt", num_timesteps=1000):
    ...
    for epoch in range(epochs):
        ...
        if avg_loss < best_loss:
            ...
            torch.save({
                ...
                'input_dim': input_dim, 'mu': mu, 'sigma': sigma, 'num_timesteps': num_timesteps
            }, save_path)
...
def evaluate_model(test_csv, checkpoint_path, K=10, Th=16, dt=0.1):
    ...
    if result is None:
        print("Error loading test data"); return
