...
def check_disk_space(required_gb=5):
    ...
    if free_gb < required_gb:
        print(f"⚠️ Low disk space: {free_gb:.1f} GB free, {required_gb} GB recommended")
    else:
...
# ------------------------------------------------------------------
# 4) Ensure videos/traffic_video.mp4 from HF dataset
# ------------------------------------------------------------------
os.makedirs("videos", exist_ok=True)
video_path = Path("videos/traffic_video.mp4")
if not video_path.exists():
    print("\\nDownloading demo video from Hugging Face dataset...")
    video_url = "https://huggingface.co/datasets/sharqascc/traffic-video-dataset/resolve/main/videos/traffic_video.mp4"
    download_with_retry(video_url, video_path, max_retries=3, description="Downloading demo video")
else:
...
if not sam3_path.exists():
    ...
    sam3_url = "https://huggingface.co/sharqascc/sam3-traffic-model/resolve/main/sam3.pt"
    download_with_retry(sam3_url, sam3_path, max_retries=3, description="Downloading SAM3 checkpoint")
else:
...
# ------------------------------------------------------------------
print("\\n=== Ready ===")
print("Video:", video_path.resolve(), "exists:", video_path.exists())
print("SAM3:", sam3_path.resolve(), "exists:", sam3_path.exists())
...
# Write colab_ready.py
with open("/content/colab_ready.py", "w", encoding="utf-8") as f:
    f.write(colab_ready_content)

print("✅ colab_ready.py created successfully at /content/colab_ready.py")

# Run it
print("\n" + "="*60)
print("🚀 RUNNING COLAB_READY.PY BOOTSTRAP")
print("="*60 + "\n")

subprocess.run([sys.executable, "/content/colab_ready.py"], check=True)
