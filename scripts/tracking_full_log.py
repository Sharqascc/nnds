...
Reads a detections CSV and generates a detailed per-track log:
  - track ID, first/last frame, number of detections
  - class distribution and mean confidence
  - frame gaps > max_gap and spatial jumps > max_jump
  - estimated pixel speed (per frame) min/mean/max
  - anomalous events logged with details

Usage:
    python scripts/tracking_full_log.py --detections outputs/tracking_e2e_300_detections.csv --output outputs/tracking_full_assessment.log
"""
import argparse
import pandas as pd
import numpy as np
