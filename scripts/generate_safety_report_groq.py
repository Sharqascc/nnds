...
def build_metrics(pet_csv):
    ...
    metrics = {
        ...
        "conflict_types": pet["conflict_type"].value_counts().to_dict()
        if "conflict_type" in pet.columns
        else {},
        "grid_cells": pet["grid_cell"].value_counts().to_dict()
        if "grid_cell" in pet.columns
        else {},
    }
    return metrics
