...
def test_overlay_conflict_info_all_positions(plotter):
    ...
    for pos in ["top-left", "top-right", "bottom-left", "bottom-right"]:
        out = plotter.overlay_conflict_info(
            frame, pet_value=0.5, ttc_value=2.0, frame_number=10, timestamp=0.5, position=pos
        )
        assert out.shape == frame.shape
...
def test_overlay_conflict_zone_custom_color(plotter):
    frame = make_frame()
    out = plotter.overlay_conflict_zone(
        frame, center=(100, 100), color=(0, 255, 0), radius=30, alpha=0.5
    )
    assert out.shape == frame.shape
...
def test_overlay_conflict_frame_success(plotter):
    ...
    with (
        ...
        patch.object(plotter, "overlay_trajectories", return_value=make_frame()),
        patch.object(plotter, "overlay_conflict_zone", return_value=make_frame()),
        patch.object(plotter, "overlay_conflict_info", return_value=make_frame()),
    ):
...
def test_overlay_conflict_frame_failure(plotter):
    ...
    with patch("cv2.VideoCapture", return_value=cap), pytest.raises(RuntimeError):
        plotter.overlay_conflict_frame(video_path="dummy.mp4", frame_idx=0, trajectories=[])
...
def test_create_before_during_after():
    with (
        patch.object(VideoOverlayPlotter, "overlay_conflict_frame", return_value=make_frame()),
        patch.object(VideoOverlayPlotter, "save_frame") as save_mock,
    ):
        create_before_during_after(
            "dummy.mp4", 0, 1, 2, [[(0, 0, 0), (1, 1, 1)]], save_path="output/seq.png"
        )
...
def test_save_conflict_frame_success(tmp_path):
    ...
    cap = make_mock_cap()
    with (
        patch("src.analysis.visualization.video_overlays.SpatialGrid", return_value=grid),
        patch("cv2.VideoCapture", return_value=cap),
        patch.object(VideoOverlayPlotter, "save_frame") as save_mock,
    ):
