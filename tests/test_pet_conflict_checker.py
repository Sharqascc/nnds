...
def test_pet_conflict_checker_filter_by_roi():
    ...
    checker = PETConflictChecker()
    df = pd.DataFrame({"x": [10.0, 20.0, 30.0, 40.0], "y": [10.0, 20.0, 30.0, 40.0]})
...
def test_conflict_result():
    """Test ConflictResult."""
    result = ConflictResult(id_a=10, id_b=20, pet=0.5, severity=ConflictSeverity.MODERATE)
...
def test_pet_uncertainty():
    ...
    uncertainty = PETUncertainty(
        nominal_pet=0.5, uncertainty_std=0.05, error_sources={"detection": 0.03, "homography": 0.02}
    )
...
def test_pet_uncertainty_ci95():
    ...
    uncertainty = PETUncertainty(nominal_pet=0.5, uncertainty_std=0.05, error_sources={})
