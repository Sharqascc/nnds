...
from src.analysis.gate_counter import VirtualGate, RobustTracker, TrafficVolumeCounter
...
def test_process_video_full(tmp_path):
    ...
    cap.read.side_effect = [(True, frame.copy()), (True, frame.copy()), (True, frame.copy()), (False, None)]

    with patch("cv2.VideoCapture", return_value=cap),          patch("cv2.VideoWriter", MagicMock()) as writer_cls,          patch("cv2.VideoWriter_fourcc", return_value=0),          patch("src.analysis.gate_counter.logger") as logger_mock:
        writer = MagicMock()
        ...
        result = counter.process_video(detector, output_video=str(tmp_path/"out.mp4"), max_frames=3, show_progress=False)
...
def test_process_video_no_output(tmp_path):
    ...
    cap.read.side_effect = [(True, np.zeros((240,320,3), dtype=np.uint8)), (True, np.zeros((240,320,3), dtype=np.uint8)), (False, None)]
