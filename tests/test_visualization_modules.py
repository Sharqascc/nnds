...
def test_load_pet_csv(tmp_path):
    ...
    csv.write_text(
        "world_traj_i,world_traj_j,event_id,pet,track_a,track_b,conflict_type\n"
...
def test_plot_conflict_event_with_mocks():
    ...
    with (
        ...
        result = plotter.plot_conflict_event(
            df, 1, save_path=None, show_conflict_zone=True, show_velocities=True
        )
...
def test_plot_conflict_event_save_path(tmp_path):
    ...
    with (
        patch("matplotlib.pyplot.subplots", return_value=(fig, ax)),
        patch("matplotlib.pyplot.tight_layout"),
        patch.object(plotter, "_save_figure", MagicMock()) as save_mock,
    ):
...
def test_diffusion_plotter_plot_all(tmp_path):
    plotter = DiffusionPETPlotter()
    with (
        patch("src.analysis.visualization.pet_diffusion_plots.plot_pet_like_histogram") as p1,
...
def test_plot_residual_analysis_direct():
    ...
    for i in range(2):
        for j in range(2):
            axes[i, j] = MagicMock()
    with (
        patch("matplotlib.pyplot.subplots", return_value=(MagicMock(), axes)),
