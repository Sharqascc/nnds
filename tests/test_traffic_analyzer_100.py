...
def test_estimate_speed_not_enough_speeds():
    ...
    pos = np.array([[0,0],[1000,1000],[2000,2000],[3000,3000],[4000,4000]], dtype=np.float32)
...
def test_run_video_to_pet_sam3_missing_weights_success(tmp_path, monkeypatch):
    ...
    class FakeResult:
        pet_events = [{"pet": 1.0, "frame_idx": 5, "track_a": 1, "track_b": 2, "conflict_type": "crossing"}]
    ...
    with mock.patch("src.analysis.grid_trajectory.sam3_grid_pet.run_sam3_grid_pet", return_value=FakeResult()):
        df = run_video_to_pet(video, bev_config_path=bev, grid_config_path=grid,
                              detector="sam3", sam3_weights_path=missing_sam3,
                              out_csv_path=out)
    assert len(df) == 1
...
def test_run_video_to_pet_yolo_cpu_success(tmp_path):
    ...
    with mock.patch("src.analysis.grid_trajectory.yolo_cpu_grid_pet.run_yolo_cpu_grid_pet",
                    return_value={"pet_events": [{"pet": 2.0, "frame": 10, "track_a": "track_3", "track_b": "track_4", "conflict_type": "rear-end"}]}):
        df = run_video_to_pet(video, bev_config_path=bev, grid_config_path=grid,
...
def test_run_video_to_pet_uvh_coco_success(tmp_path):
    ...
    with mock.patch("src.analysis.grid_trajectory.uvh_coco_fused_grid_pet.run_uvh_coco_fused_grid_pet",
...
def test_run_video_to_pet_rtdetr_not_implemented(tmp_path):
    ...
    with pytest.raises(NotImplementedError, match="RT-DETR video pipeline is not implemented"):
        run_video_to_pet(video, bev_config_path=bev, grid_config_path=grid,
                         detector="rtdetr", rtdetr_weights_path=rtdetr)
...
def test_run_video_to_pet_dict_events_branch(tmp_path):
    ...
    with mock.patch("src.analysis.grid_trajectory.sam3_grid_pet.run_sam3_grid_pet", return_value=type("Result", (), {"pet_events": [event]})):
        df = run_video_to_pet(video, bev_config_path=bev, grid_config_path=grid,
                              detector="sam3", sam3_weights_path=sam3,
                              out_csv_path=out)
    assert len(df) == 1
...
def test_main_runs_pipeline(monkeypatch, tmp_path):
    video = _dummy_file(tmp_path / "video.mp4")
    monkeypatch.setattr(sys, 'argv', ['traffic_analyzer.py', '--video', str(video), '--detector', 'sam3'])
...
def test_run_video_to_pet_fixed_success(tmp_path):
    ...
    class FakeResult:
        pet_events = [{"pet": 1.2, "frame_idx": 30, "track_a": 10, "track_b": 11, "conflict_type": "crossing"}]

    with mock.patch("src.analysis.grid_trajectory.sam3_grid_pet.run_sam3_grid_pet", return_value=FakeResult()):
        df = run_video_to_pet_fixed(str(video), bev_config_path=str(bev),
