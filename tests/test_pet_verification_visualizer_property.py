...
@given(
    st.one_of(
        st.lists(
            st.dictionaries(keys=st.just("x_pixel"), values=st.integers(0, 2000))
            | st.dictionaries(keys=st.just("y_pixel"), values=st.integers(0, 2000))
            | st.dictionaries(
                keys=st.sampled_from(["x_pixel", "y_pixel"]), values=st.integers(0, 2000)
            ),
            min_size=1,
            max_size=10,
        ),
...
@given(
    st.lists(
        st.dictionaries(keys=st.sampled_from(["x_pixel", "y_pixel"]), values=st.integers(0, 2000)),
        min_size=3,
        max_size=20,
    )
)
