...
def test_batch_logger_and_skip(tmp_path):
    ...
    traj_b = pd.DataFrame({'timestamp': [100.0], 'frame': [100], 'x': [10], 'y': [10], 'track_id': [2]})
...
def test_estimate_velocity_savgol_even_window():
    checker = PETConflictChecker(enable_logging=False)
    traj = pd.DataFrame({'x': [0,1,2,3,4,5,6], 'y': [0,1,2,3,4,5,6], 'timestamp': [0,1,2,3,4,5,6]})
...
def test_estimate_velocity_savgol_no_finite():
    checker = PETConflictChecker(enable_logging=False)
    traj = pd.DataFrame({'x': [0,1,2,3,4,5,6], 'y': [0,1,2,3,4,5,6], 'timestamp': [0,1,2,3,4,5,6]})
    with patch('src.analysis.pet_conflict_checker.savgol_filter', return_value=np.array([np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan])):
...
def test_cli_main_guard(monkeypatch, tmp_path):
    ...
    with patch('src.analysis.pet_conflict_checker.PETConflictChecker.detect_from_csv', return_value=pd.DataFrame(columns=['pet'])):
