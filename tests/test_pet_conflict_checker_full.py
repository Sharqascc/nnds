...
def test_detect_conflicts_no_velocity():
    ...
    assert len(out) == 2
    assert out["pet_uncertainty_std"].iloc[0] == pytest.approx(
        np.sqrt(0.2**2 + 0.3**2 + 0.1**2) / 5.0
    )
...
def test_checker_detect_from_trajectories_batch():
    checker = PETConflictChecker(enable_logging=False, enable_uncertainty=True)
    traj_a = pd.DataFrame(
        {"track_id": [1, 1], "frame": [0, 1], "timestamp": [0.0, 0.1], "x": [0, 1], "y": [0, 1]}
    )
    traj_b = pd.DataFrame(
        {"track_id": [2, 2], "frame": [0, 1], "timestamp": [0.2, 0.3], "x": [2, 3], "y": [2, 3]}
    )
    pairs = [(traj_a, traj_b)]
...
def test_conflict_result_to_dict():
    ...
    cr = ConflictResult(
        1, 2, 1.0, ConflictSeverity.MODERATE, unc, frame_start=0, frame_end=10, extra={"site": "X"}
    )
    d = cr.to_dict()
    assert d["id_a"] == 1
...
def test_detect_from_trajectories_batch_logger_and_skip_and_exception():
    ...
    with patch(
        "src.analysis.pet_conflict_checker.compute_pet", side_effect=[3.0, Exception("boom")]
    ):
        results = checker.detect_from_trajectories_batch(pairs, fps=10)
...
def test_estimate_velocity_all_invalid_speeds():
    checker = PETConflictChecker(enable_logging=False)
    traj = pd.DataFrame(
        {"x": [0.0, np.nan, 2.0], "y": [0.0, np.nan, 2.0], "timestamp": [0.0, 1.0, 2.0]}
    )
    assert checker._estimate_velocity(traj) == 5.0
...
def test_estimate_velocity_savgol_window_adjust_and_fallback():
    ...
    traj = pd.DataFrame({"x": [0.0, 1.0, 2.0], "y": [0.0, 1.0, 2.0], "timestamp": [0.0, 1.0, 2.0]})
    assert checker._estimate_velocity_savgol(
        traj, window=2, polyorder=2
    ) == checker._estimate_velocity(traj)
...
def test_estimate_velocity_savgol_return_nan_for_invalid_speeds():
    ...
    with patch(
        ...
        side_effect=[np.array([np.nan] * 5), np.array([np.nan] * 5)],
    ):
