...
def test_compute_sample_size_two_sided():
    uq = UncertaintyQuantifier()
    n = uq.compute_required_sample_size(effect_size=0.5, power=0.8, alpha=0.05, test_type="two-sided")
    assert n > 0
    assert isinstance(n, int)

def test_compute_sample_size_one_sided():
    uq = UncertaintyQuantifier()
    n = uq.compute_required_sample_size(effect_size=0.5, power=0.8, alpha=0.05, test_type="one-sided")
    assert n > 0
