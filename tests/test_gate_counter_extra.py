...
def test_process_video_gate_crossing_and_draw_tracks(tmp_path):
    ...
    detections = [[{'centroid': (5,5), 'cls': 'car', 'conf': 0.9, 'track_id': 1}], [{'centroid': (5,-5), 'cls': 'car', 'conf': 0.9, 'track_id': 1}]]
    call_count = {'n': 0}
    def detector(frame):
        idx = call_count['n']
        call_count['n'] += 1
...
def test_process_video_draw_tracks_branch(tmp_path):
    ...
    with patch('cv2.VideoCapture', return_value=cap), \
        ...
        counter.process_video(detector=lambda f: [det], max_frames=2, output_video=str(tmp_path/'out.mp4'))
