...
def test_real_calibration_missing_bev(tmp_path):
    calib_path, bev_path = make_calib_files(tmp_path, {"pixel_points":[], "world_points":[]}, None)
    result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is None

def test_real_calibration_insufficient_points(tmp_path):
    calib_data = {"pixel_points":[[0,0]], "world_points":[[0,0]]}
    bev_data = {"bounds": {"x_min":0,"x_max":100,"y_min":0,"y_max":100}, "resolution": [100,100]}
    calib_path, bev_path = make_calib_files(tmp_path, calib_data, bev_data)
    result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is None

def test_real_calibration_shape_mismatch(tmp_path):
    calib_data = {"pixel_points":[[0,0],[1,0],[0,1],[1,1]], "world_points":[[0,0],[1,0],[0,1],[1,1],[2,2]]}
    bev_data = {"bounds": {"x_min":0,"x_max":100,"y_min":0,"y_max":100}, "resolution": [100,100]}
    calib_path, bev_path = make_calib_files(tmp_path, calib_data, bev_data)
    result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is None
...
def test_real_calibration_pixel_not_2d(tmp_path):
    calib_data = {"pixel_points":[[0,0,0],[1,0,0],[0,1,0],[1,1,0]], "world_points":[[0,0],[1,0],[0,1],[1,1]]}
    bev_data = {"bounds": {"x_min":0,"x_max":100,"y_min":0,"y_max":100}, "resolution": [100,100]}
    calib_path, bev_path = make_calib_files(tmp_path, calib_data, bev_data)
    result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is None
...
def test_real_calibration_missing_bounds_res(tmp_path):
    calib_data = {"pixel_points":[[0,0],[1,0],[0,1],[1,1]], "world_points":[[0,0],[1,0],[0,1],[1,1]]}
    bev_data = {}
    calib_path, bev_path = make_calib_files(tmp_path, calib_data, bev_data)
    result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is None
...
def test_real_calibration_bounds_missing_keys(tmp_path):
    ...
    with patch('cv2.findHomography', return_value=(np.eye(3), np.ones((4,1), dtype=np.uint8))),          patch('src.bev.bev_mapper.BEVMapper', DummyMapper):
        result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is None
...
def test_real_calibration_uncertainty_with_method(tmp_path):
    ...
    with patch('cv2.findHomography', return_value=(np.eye(3), np.ones((4,1), dtype=np.uint8))),          patch('src.bev.bev_mapper.BEVMapper', MockMapper):
        result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is not None
...
def test_main_success(tmp_path, capsys):
    ...
    with patch('sys.argv', ['prog', '--calib', calib_path, '--bev-config', bev_path]),          patch('cv2.findHomography', return_value=(np.eye(3), np.ones((4,1), dtype=np.uint8))):
        with pytest.raises(SystemExit) as e:
            main()
        assert e.value.code == 0

def test_main_failure_none(tmp_path):
    with patch('sys.argv', ['prog', '--calib', str(tmp_path/"missing.json"), '--bev-config', str(tmp_path/"missing_bev.json")]):
        with pytest.raises(SystemExit) as e:
            main()
...
def test_main_quiet(tmp_path):
    ...
    bev_data = {"bounds": {"x_min":0,"x_max":100,"y_min":0,"y_max":100}, "resolution": [100,100]}
    calib_path, bev_path = make_calib_files(tmp_path, calib_data, bev_data)
    with patch('sys.argv', ['prog', '--calib', calib_path, '--bev-config', bev_path, '--quiet']),          patch('cv2.findHomography', return_value=(np.eye(3), np.ones((4,1), dtype=np.uint8))):
        with pytest.raises(SystemExit) as e:
            main()
        assert e.value.code == 0
...
def test_main_raise_on_failure(tmp_path):
    ...
    with patch('sys.argv', ['prog', '--calib', 'dummy', '--bev-config', 'dummy', '--raise-on-failure']),          patch('src.bev.bev_mapper.test_with_real_calibration', return_value={"passes_validation": False, "rmse_m": 2.0}):
        with pytest.raises(RuntimeError):
            main()
...
def test_real_calibration_no_valid_projection(tmp_path):
    ...
    bev_data = {"bounds": {"x_min":0,"x_max":100,"y_min":0,"y_max":100}, "resolution": [100,100]}
    calib_path, bev_path = make_calib_files(tmp_path, calib_data, bev_data)
    with patch('cv2.findHomography', return_value=(np.eye(3), np.ones((4,1), dtype=np.uint8))),          patch('src.bev.bev_mapper._project_points_homography', return_value=np.full((4,2), np.nan)):
        result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is None
...
def test_real_calibration_quality_branches(tmp_path):
    ...
    for rmse, expected in [(0.3, "GOOD"), (0.7, "ACCEPTABLE"), (2.0, "POOR - RECALIBRATION RECOMMENDED")]:
        calib_data = {"pixel_points":[[0,0],[1,0],[0,1],[1,1]], "world_points":[[0,0],[1,0],[0,1],[1,1]]}
        bev_data = {"bounds": {"x_min":0,"x_max":100,"y_min":0,"y_max":100}, "resolution": [100,100]}
        calib_path, bev_path = make_calib_files(tmp_path, calib_data, bev_data)
        ...
        world_reproj = np.array([[0,0],[1,0],[0,1],[1+rmse*2,1]])  # one point off by rmse*2
        with patch('cv2.findHomography', return_value=(np.eye(3), np.ones((4,1), dtype=np.uint8))),              patch('src.bev.bev_mapper._project_points_homography', return_value=world_reproj):
            result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
        assert result is not None
...
def test_real_calibration_inverse_failure(tmp_path):
    ...
    with patch('cv2.findHomography', return_value=(np.eye(3), np.ones((4,1), dtype=np.uint8))),          patch('numpy.linalg.inv', side_effect=Exception("inverse failed")):
        result = test_with_real_calibration(calib_json=calib_path, bev_json=bev_path)
    assert result is not None
...
def test_main_validation_failure_exit_1(tmp_path):
    ...
    with patch('sys.argv', ['prog', '--calib', 'dummy', '--bev-config', 'dummy']),          patch('src.bev.bev_mapper.test_with_real_calibration', return_value={"passes_validation": False, "rmse_m": 2.0}):
        with pytest.raises(SystemExit) as e:
            main()
        assert e.value.code == 1
