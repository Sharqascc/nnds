...
def test_parse_args_custom(monkeypatch):
    monkeypatch.setattr(sys, 'argv', [
        'prog', '--num-trials', '5', '--seed', '123', '--plot',
        '--multi-noise', '--verbose'
    ])
    parser = parse_args()
    args = parser.parse_args()
...
def test_solve_pnp_world_error_success():
    world = np.zeros((5,3), dtype=np.float32)
    img = np.zeros((5,2), dtype=np.float32)
    with patch('cv2.solvePnP', return_value=(True, np.zeros(3), np.zeros(3))),          patch('cv2.Rodrigues', return_value=(np.eye(3, dtype=np.float32), None)),          patch('src.bev.calibration.monte_carlo_calibration_benchmark.world_from_pnp', return_value=world[:, :2]),          patch('src.bev.calibration.monte_carlo_calibration_benchmark.mae_world', return_value=0.123):
        mae, R, t = solve_pnp_world_error(world, img, 0)
    assert mae == 0.123
...
def test_solve_pnp_Z0_world_error():
    ...
    with patch('src.bev.calibration.monte_carlo_calibration_benchmark.solve_pnp_world_error', return_value=(0.5, None, None)) as mock_solve:
...
def test_p3p_with_failures_and_success():
    ...
    img = np.zeros((5,2), dtype=np.float32)
    rng = np.random.default_rng(0)
    with patch('cv2.SOLVEPNP_P3P', 1, create=True),          patch('cv2.solvePnP', side_effect=[(False, None, None), (True, np.zeros(3), np.zeros(3))]),          patch('cv2.Rodrigues', return_value=(np.eye(3, dtype=np.float32), None)),          patch('src.bev.calibration.monte_carlo_calibration_benchmark.world_from_pnp', return_value=world[:, :2]),          patch('src.bev.calibration.monte_carlo_calibration_benchmark.mae_world', return_value=0.8):
...
def test_run_single_trial(monkeypatch):
    ...
    monkeypatch.setattr(mc, 'add_plane_bias', MagicMock(return_value=np.zeros((27*12,3), dtype=np.float32)))
    monkeypatch.setattr(mc, 'project_points', MagicMock(return_value=np.zeros((27*12,2), dtype=np.float32)))
    monkeypatch.setattr(mc, 'add_anisotropic_noise_2d', MagicMock(return_value=np.zeros((27*12,2), dtype=np.float32)))
    monkeypatch.setattr(mc, 'estimate_homography', MagicMock(side_effect=[(np.eye(3), None), (np.eye(3), None)]))
    monkeypatch.setattr(mc, 'apply_homography', MagicMock(return_value=np.zeros((27*12,2), dtype=np.float32)))
...
def test_export_summary(tmp_path):
    ...
    summary = export_summary(
        out, num_trials=3, sigma_px_x=1.0, sigma_px_y=2.0, rho_noise=0.5, plane_bias_cm=1.5,
        mae_H_biased=arr, mae_H_Z0=arr, mae_PNP=arr, mae_PNP_Z0=arr, mae_P3P=arr
    )
    assert out.exists()
...
def test_maybe_plot_mae_hist(tmp_path):
    ...
    with patch('matplotlib.pyplot.figure'),          patch('matplotlib.pyplot.hist'),          patch('matplotlib.pyplot.xlabel'),          patch('matplotlib.pyplot.ylabel'),          patch('matplotlib.pyplot.title'),          patch('matplotlib.pyplot.grid'),          patch('matplotlib.pyplot.tight_layout'),          patch('matplotlib.pyplot.savefig'),          patch('matplotlib.pyplot.close'):
...
def test_main_single_scenario(monkeypatch, tmp_path):
    import src.bev.calibration.monte_carlo_calibration_benchmark as mc
    monkeypatch.setattr(sys, 'argv', ['prog', '--num-trials', '1', '--output-summary', str(tmp_path/'summary.json')])
    monkeypatch.setattr(mc, 'parse_args', lambda: SimpleNamespace(num_trials=1, seed=0, plot=False, multi_noise=False, output_summary=tmp_path/'summary.json', verbose=False))
...
def test_main_multi_noise(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sys, 'argv', ['prog', '--multi-noise', '--num-trials', '1', '--output-summary', str(output_summary)])
    monkeypatch.setattr(mc, 'parse_args', lambda: SimpleNamespace(num_trials=1, seed=0, plot=False, multi_noise=True, output_summary=output_summary, verbose=False))
    monkeypatch.setattr(mc, 'configure_logging', MagicMock())
    monkeypatch.setattr(mc, 'run_single_scenario', MagicMock(return_value={'test': 'ok'}))
    main()
    assert mc.run_single_scenario.call_count == 4  # noise_levels length 4
    # Index file should be created
    index_path = output_summary.with_suffix("").with_name(output_summary.with_suffix("").name + "_index.json")
