...
class SSMVerifier:
    ...
    def verify_pet_calculation(
        ...
        results["statistics"]["severity_distribution"] = {
            "critical": {
                "count": int(critical),
                "percentage": float(100 * critical / data.size),
            },
            "serious": {
                "count": int(serious),
                "percentage": float(100 * serious / data.size),
            },
            "moderate": {
                "count": int(moderate),
                "percentage": float(100 * moderate / data.size),
            },
        }
    ...
    def verify_ttc_calculation(
        ...
        results["statistics"]["severity_distribution"] = {
            "imminent": {
                "count": int(imminent),
                "percentage": float(100 * imminent / data.size),
            },
            "critical": {
                "count": int(critical),
                "percentage": float(100 * critical / data.size),
    ...
    def verify_drac_calculation(
        self, drac_values: np.ndarray, expected_range: tuple[float, float] = (0.0, 10.0)
    ) -> dict:
        """
...
def compare_with_reference(
    observed: np.ndarray, reference: np.ndarray, metric_name: str = "SSM"
) -> dict:
    """
    ...
    """
    results = {"metric": metric_name, "tests": {}, "passed": True}
