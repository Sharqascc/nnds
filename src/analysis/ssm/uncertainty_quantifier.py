...
class UncertaintyQuantifier:
    ...
    def _parametric_ci(self, data: np.ndarray) -> tuple[float, float]:
        ...
        return stats.t.interval(self.confidence_level, df=n - 1, loc=mean, scale=sem)
    ...
    def _bca_ci(
        self, data: np.ndarray, bootstrap_stats: np.ndarray, statistic: Callable
    ) -> tuple[float, float]:
            ...
            for i in range(self.n_bootstrap):
                ...
                if len(boot_g1) > 0 and len(boot_g2) > 0:
                    if estimator == "cohens_d":
                        ...
                        bootstrap_d[i] = (
                            (np.mean(boot_g1) - np.mean(boot_g2)) / pooled_std_boot
...
def compute_confidence_interval(
    data: np.ndarray, confidence: float = 0.95, method: str = "parametric"
) -> tuple[float, float]:
    """Compute confidence interval using specified method."""
    uq = UncertaintyQuantifier(confidence_level=confidence)
...
def compute_effect_size(
    group1: np.ndarray, group2: np.ndarray | None = None, estimator: str = "cohens_d"
) -> float:
    """
