...
class SSMPlotter:
    ...
    def __init__(
        ...
        self.thresholds = {
            ...
            "ttc": {"critical": critical_ttc, "warning": warning_ttc, "safe": safe_ttc},
        }
    ...
    def validate_ssm_data(
        self, data: np.ndarray, metric_name: str, allow_negative: bool = False
    ) -> dict:
        """
        ...
        if data.ndim != 1:
            ...
            results["errors"].append(
                f"{metric_name}: Data must be 1D array (got shape {data.shape})"
            )
            return results
    ...
    def plot_pet_distribution(
        ...
        threshold_lines = {
            f"Critical (<{pet_thresh['critical']}s)": (
                pet_thresh["critical"],
                self.COLORS["red"],
            ),
            f"Serious (<{pet_thresh['serious']}s)": (
                pet_thresh["serious"],
                self.COLORS["orange"],
            ),
            f"Moderate (<{pet_thresh['moderate']}s)": (
                pet_thresh["moderate"],
                self.COLORS["yellow"],
            ),
            f"Safe (>{pet_thresh['safe']}s)": (
                pet_thresh["safe"],
                self.COLORS["green"],
            ),
        }

        for val, color in threshold_lines.values():
        ...
        legend_elements = [
            mpatches.Patch(
                color=self.COLORS["red"], label=f"Critical (<{pet_thresh['critical']}s)"
            ),
            mpatches.Patch(
                color=self.COLORS["orange"],
                label=f"Serious ({pet_thresh['critical']}-{pet_thresh['serious']}s)",
            ),
            mpatches.Patch(
    ...
    def plot_comparative_boxplot(
        ...
        ax.boxplot(
            ...
            boxprops={"facecolor": self.COLORS["blue"], "alpha": 0.7},
    ...
    def plot_conflict_density_map(
        self,
        pet_values: np.ndarray,
        custom_bands: list[float] | None = None,
        save_path: str | None = None,
    ) -> plt.Figure:
        """
    ...
    def plot_cumulative_distribution(
        ...
        if metric_name.upper() == "PET":
            ...
            ax.axvline(
                ...
                linewidth=1.5,
                alpha=0.7,
                label=f"Serious ({thresh['serious']}s)",
            )
            ax.axvline(
                thresh["safe"],
                color="green",
            ...
            ax.axvline(
                ...
                linewidth=1.5,
                alpha=0.7,
                label=f"Critical ({thresh['critical']}s)",
            )
            ax.axvline(
                thresh["safe"],
                color="green",
...
def plot_pet_distribution(
    pet_values: np.ndarray, style: str = "journal", save_path: str | None = None
) -> plt.Figure:
...
def plot_correlation_heatmap(
    data_dict: dict[str, np.ndarray], save_path: str | None = None
) -> plt.Figure:
    """Correlation heatmap for multiple SSM metrics."""
    if not HAS_PANDAS:
        raise ImportError(
            "pandas required for correlation heatmap. Install with: pip install pandas"
        )
