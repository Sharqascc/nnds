...
class ReproducibilityAuditor:
    ...
    def log_input(self, name: str, path: str, metadata: dict | None = None):
    ...
    def log_output(self, name: str, path: str, metadata: dict | None = None):
    ...
    def generate_report(self, save_path: str | None = None) -> dict:
        """Generate comprehensive reproducibility report."""
        # Compute total execution time
        if self.session_start:
            total_duration = datetime.now() - self.session_start
            self.session_data["execution"]["total_duration_seconds"] = (
    ...
    def verify_reproducibility(self, saved_report_path: str) -> dict:
        ...
        if saved_py != current_py:
            mismatches.append(
                f"Python version: saved={saved_py[:30]}... vs current={current_py[:30]}..."
            )
        ...
        for pkg, saved_ver in saved_pkgs.items():
            ...
            if pkg in current_pkgs:
                if current_pkgs[pkg] != saved_ver:
                    mismatches.append(
                        f"Package {pkg}: saved={saved_ver} vs current={current_pkgs[pkg]}"
                    )
            else:
        ...
        if saved_commit != current_commit and saved_commit != "unknown":
            mismatches.append(
                f"Git commit: saved={saved_commit[:8]} vs current={current_commit[:8]}"
            )
...
def audit_environment(save_path: str | None = None) -> dict:
...
def generate_audit_report(
    config: dict | None = None, save_path: str | None = None, description: str = ""
) -> dict:
