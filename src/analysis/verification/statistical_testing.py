...
class StatisticalTester:
    ...
    def check_normality(self, data: np.ndarray, method: str = "shapiro") -> dict:
        """
        Test for normality.
    ...
    def check_homoscedasticity(self, *groups: np.ndarray) -> dict:
        """
    ...
    def check_assumptions(self, *groups: np.ndarray, test_type: str = "t-test") -> dict:
        """
        ...
        # Two-sample t-test
            ...
            if paired:
                ...
                if equal_var:
                    ...
                    cohens_d = (
                        (np.mean(g1) - np.mean(g2)) / np.std(g2, ddof=1)
                        if np.std(g2, ddof=1) > 0
                        else 0
                    )
            ...
            ci2 = stats.t.interval(
                1 - self.alpha, df=len(g2) - 1, loc=np.mean(g2), scale=stats.sem(g2)
            )
...
def paired_test(
    before: np.ndarray, after: np.ndarray, alpha: float = 0.05, parametric: bool = True
) -> dict:
