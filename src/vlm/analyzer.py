...
class VLLMAnalyzer:
    """Enhanced VLM analyzer using small BLIP VQA model."""

    def __init__(self, model_name="Salesforce/blip-vqa-base", device="cuda"):
    ...
    def analyze_with_trajectory(
        ...
        if trajectory_df is not None and not trajectory_df.empty:
            summary = {
                "num_vehicles": len(trajectory_df["track_id"].unique())
                if "track_id" in trajectory_df
                else len(trajectory_df),
                "avg_speed": trajectory_df["speed"].mean()
                if "speed" in trajectory_df
                else None,
                "max_speed": trajectory_df["speed"].max()
                if "speed" in trajectory_df
                else None,
                "pet_events": len(trajectory_df[trajectory_df.get("pet", 0) < 2.0])
                if "pet" in trajectory_df
                else 0,
                "ttc_events": len(trajectory_df[trajectory_df.get("ttc", 10) < 3.0])
                if "ttc" in trajectory_df
                else 0,
            }
            summary_str = ", ".join(
...
Output your answer as a JSON object with keys: risk_level, concerns (list of strings), causes (list of strings), suggestions (list of strings)."""
        ...
        inputs = {
            ...
            if k in ["pixel_values", "input_ids"]
        }
        with torch.no_grad():
